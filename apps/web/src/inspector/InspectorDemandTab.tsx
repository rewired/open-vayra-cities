  ...
  return (
    <section className="inspector-demand-tab" aria-label="Demand">
      ...
      {scenarioDemandProvenanceProjection.status === 'ready' && (
        <InspectorDisclosure summaryText="Demand model">
          <div className="inspector-demand-provenance">
            <p className="inspector-dialog__note">{scenarioDemandProvenanceProjection.summary}</p>
            <p className="inspector-dialog__note inspector-dialog__note--caveat">
              {scenarioDemandProvenanceProjection.modelCaveat}
            </p>
            <p className="inspector-dialog__note">
              {scenarioDemandProvenanceProjection.stopBoundaryNote}
            </p>
            
            {scenarioDemandProvenanceProjection.sourceRows.length > 0 && (
              <table className="inspector-compact-table">
                ...
                <tbody>
                  {scenarioDemandProvenanceProjection.sourceRows.map((row, idx) => (
                    <tr key={idx}>
                      <th scope="row">{row.sourceKindLabel}</th>
                      <td>{row.label}</td>
                      <td className="inspector-compact-table__value--left">
                        {[row.datasetYearLabel, row.sourceDateLabel, row.licenseHint].filter(Boolean).join(', ') || '-'}
                        {row.attributionHint && <div className="inspector-demand-provenance__attribution">© {row.attributionHint}</div>}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <p className="inspector-dialog__note inspector-demand-provenance__generator">
              Generated by: {scenarioDemandProvenanceProjection.generatorLabel}
            </p>
          </div>
        </InspectorDisclosure>
      )}
