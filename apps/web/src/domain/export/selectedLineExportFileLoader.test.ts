...
describe('selectedLineExportFileLoader', () => {
  it('parses valid JSON text safely', () => {
    const result = parseSelectedLineExportJsonText('{"schemaVersion":"openvayra-cities-selected-line-export-v3"}');

    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.parsed).toEqual({ schemaVersion: 'openvayra-cities-selected-line-export-v3' });
    }
  });
